...
export class DbUpdateSagaStep implements UpdateSagaStep {
  constructor(
    private readonly updateSagaStateRepository: UpdateSagaStateRepository,
  ) {}

  async updateStep(
    params: UpdateSagaStep.Params,
  ): Promise<UpdateSagaStep.Result> {
    const sagaState = await this.updateSagaStateRepository.update(params);
    if (sagaState.status === SagaStatus.SagaCompleted) {
